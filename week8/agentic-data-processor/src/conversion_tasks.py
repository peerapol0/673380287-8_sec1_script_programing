            ...
            for key, value in row.items():
                try:
                    ...
                    if stripped.lstrip('-').isdigit():
                        json_row[key] = int(stripped)
                    elif stripped.lstrip('-').replace('.', '', 1).isdigit():
                        json_row[key] = float(stripped)
                    elif stripped.lower() in ['true', 'false']:
                        json_row[key] = stripped.lower() == 'true'
                    elif stripped.lower() == 'null' or stripped == '':
                        json_row[key] = None
                    else:
                        json_row[key] = stripped
                except Exception as e:
                    logger.debug(
                        f"Could not convert value '{value}' for key '{key}' to numeric/boolean type. Keeping as string. Error: {e}")
                    json_row[key] = value.strip() if isinstance(value, str) else value
