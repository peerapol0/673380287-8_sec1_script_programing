...
class JSONTasks:
    ...
    def update_json_data(self, data, updates):
        ...
        Handles nested paths. A path of "" applied with operation 'add_to_list' appends
        directly to the top-level list.
        """
        if not data:
        ...
        for update in updates:
            ...
            try:
                # Special case: empty path with add_to_list means "append to the root list"
                if path == "" and operation == "add_to_list":
                    if isinstance(modified_data, list):
                        modified_data.append(value)
                        logger.info(f"Added item to root list. New length: {len(modified_data)}.")
                    else:
                        logger.warning("Cannot 'add_to_list' at root: root data is not a list. Skipping update.")
                    continue
